//9.5 inch screen with HDMI Driver
...
inside_width  = 126+8;             //Inside open space in mm for the width
inside_length = 210+8;             //Inside open space in mm for the length
...
do_pcb_mounts = 0;              // PC Board Standoffs
do_lid_holes  = 0;              // Drill holes for controls in the lid
do_usb_hole   = 0;              // USB jack hole to put cable into a board
do_display    = 0;              // Hole for display (number is display type, 1=1.3" OLED)
// -----------------------------------------------------------------------------------
...
oledWinWidth  = 126;             // Side to side window size
oledWinHeight = 210;             // Top to bottom windown size
oledOriginX   = 0;             // Places OLED display with 1 corner standoff
oledOriginY   = 0; // Places OLED display 1st standoff
...
translate([-outside_width-10,0,0]) {
    difference() {
        ...
        if (do_display > 0) {
            ...
            oledWindowHole(oledOriginX, oledOriginY, oledWinWidth, oledWinHeight);
        }
